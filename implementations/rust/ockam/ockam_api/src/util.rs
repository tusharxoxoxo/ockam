...
pub mod test_utils {
    ...
    use ockam::identity::{CredentialsMemoryRetriever, SecureChannels};
    use ockam::Result;
    ...
    pub async fn start_manager_for_tests(context: &mut Context) -> Result<NodeManagerHandle> {
        ...
        let attributes = AttributesBuilder::with_schema(PROJECT_MEMBER_SCHEMA)
            .with_attribute("role".as_bytes(), "user".as_bytes())
            .build();
        ...
        let credential_retriever = Arc::new(CredentialsMemoryRetriever::new(credential));
        ...
        let node_manager = InMemoryNode::new(
            ...
            NodeManagerTrustOptions::new(Some(credential_retriever), Some(identifier.clone())),
        )
        .await?;

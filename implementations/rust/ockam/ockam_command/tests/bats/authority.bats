...
@test "authority - an authority node must be shown as UP even if its tcp listener cannot be accessed" {
  ...
  trusted="{}"
  run_success "$OCKAM" authority create --tcp-listener-address="127.0.0.1:$port" --trusted-identities "$trusted"
...
@test "authority - an authority identity is created by default for the authority node - with a given name" {
  ...
  trusted="{}"
  run_success "$OCKAM" authority create --tcp-listener-address="127.0.0.1:$port" --trusted-identities "$trusted" --identity ockam
...
@test "authority - standalone authority, enrollers, members" {
  ...
  run_success "$OCKAM" authority create --tcp-listener-address="127.0.0.1:$port" --trusted-identities "$trusted" --no-direct-authentication
...
EOF
  ...
  run_success "$OCKAM" project enroll --force $token1 --identity m2
  ...
  run_success "$OCKAM" project enroll --force $token2 --identity m3
